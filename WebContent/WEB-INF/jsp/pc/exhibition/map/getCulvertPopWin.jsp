...
<style>
 	/* .ex_main_rightheader_pw4_header{
	 	background:url(${ctx}/resources/image/exhibition/map/legend/ex_map_bridge.png) left 10px top 29px #00b9f6 no-repeat;
 	} */
 	.ex_main_rightheader_pw4_header{
...
<div class="ex_main_rightheader_pw4_header">
	<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_culvert.png">
	<span>${partCulvert.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
		<c:choose>
		   <c:when  test="${empty partCulvert.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${partCulvert.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${partCulvert.photosNum}</div>
					</a>
		   		</c:forEach>
	...
	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>位置:</p><span>${partCulvert.descOrgName}${partCulvert.orgName}</span></li>
		<li><p>管理单位:</p><span>${partCulvert.orgName}</span></li>	
		<li><p>联系电话:</p><span>${partCulvert.telephone}</span></li>
		<li><p>所在线路:</p>
			<span>
		...
		<li><p>穿跨形式:</p>
			<span>${partCulvert.crossSpanName}</span>
		</li>
	</ul>
</div>
<div class="imgAll">
