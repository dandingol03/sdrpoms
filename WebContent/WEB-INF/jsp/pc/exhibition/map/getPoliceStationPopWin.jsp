...
<div class="ex_main_rightheader_pw4_header">
	<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_PolicePtation.png">
	<span>${policeStation.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
   		<c:choose>
		   <c:when  test="${empty policeStation.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${policeStation.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${policeStation.photosNum}</div>
					</a>
		   		</c:forEach>
	...
	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>地址:</p><span>${policeStation.adress}</span></li>
		<li><p>管辖范围:</p><span>${policeStation.defenceBroadcast}</span></li>	
		<li><p>联系电话:</p><span>${policeStation.telephone}</span></li>
	</ul>
</div>
