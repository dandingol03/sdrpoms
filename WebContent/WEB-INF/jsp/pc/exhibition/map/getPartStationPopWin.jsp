...
<style>
 	/* .ex_main_rightheader_pw4_header{
	 	background:url(${ctx}/resources/image/exhibition/map/legend/ex_map_PartStation.png) left 10px top 29px #00b9f6 no-repeat;
 	} */
 	.ex_main_rightheader_pw4_header{
...
<div class="ex_main_rightheader_pw4_header">
	<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_PartStation.png">
	<span>${partStation.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
  		<c:choose>
		   <c:when  test="${empty partStation.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${partStation.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${partStation.photosNum}</div>
					</a>
		   		</c:forEach>
	...
	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>地址:</p><span>${partStation.descOrgName}${partStation.orgName}</span></li>
		<li><p>所属单位:</p><span>${partStation.orgName}</span></li>	
		<li><p>联系电话:</p><span>${partStation.telephone}</span></li>
		<li><p>所在线路:</p>
			<span>
	...
	<c:choose>
		<c:when test="${partStation.nature == 04 }">
			<div style="display:none;"class="machine" onclick="setPartStationX('${partStation.id}')">进站信号机</div>
		</c:when>
		<c:otherwise>
			<div style="display:block;"class="machine" onclick="setPartStationX('${partStation.id}')">进站信号机</div>
		</c:otherwise>
</c:choose>
</div>
