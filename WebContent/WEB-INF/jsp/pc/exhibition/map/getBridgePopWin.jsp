...
<div class="ex_main_rightheader_pw4_header">
	<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_bridge.png">
	<span>${partBridge.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
   		<c:choose>
		   <c:when  test="${empty partBridge.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${partBridge.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${partBridge.photosNum}</div>
					</a>
		   		</c:forEach>
	...
	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>位置:</p><span>${partBridge.descOrgName}${partBridge.orgName}</span></li>
		<li><p>管理单位:</p><span>${partBridge.orgName}</span></li>	
		<li><p>联系电话:</p><span>${partBridge.telephone}</span></li>
		<li><p>所在线路:</p>
			<span>
				<c:forEach items="${nearByRails}" var="nearByRails">
		...
		<li><p>穿跨形式:</p><span>${partBridge.crossSpanName}</span>
		</li>
	</ul>
</div>
<div class="imgAll">
