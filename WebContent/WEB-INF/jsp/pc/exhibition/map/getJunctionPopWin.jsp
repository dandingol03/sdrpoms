...
<style>
 	/* .ex_main_rightheader_pw4_header{
	 	background:url(/sdrpoms/resources/image/exhibition/map/legend/ex_map_junction.png) left 10px top 28px #00b9f6 no-repeat
 	} */
 	.ex_main_rightheader_pw4_header{
...
<div class="ex_main_rightheader_pw4_header">
	<img src="/sdrpoms/resources/image/exhibition/map/legend/ex_map_junction.png">
	<span>${partJunction.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
   		<c:choose>
		   <c:when  test="${empty partJunction.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${partJunction.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${partJunction.photosNum}</div>
					</a>
		   		</c:forEach>
	...
	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>位置:</p><span>${partJunction.descOrgName}${partJunction.orgName}</span></li>
		<li><p>管理单位:</p><span>${partJunction.orgName}</span></li>	
		<li><p>联系电话:</p><span>${partJunction.telephone}</span></li>
		<li><p>所在线路:</p>
			<span>
		...
		<li><p>相交道路:</p><span></span></li>
		<li><p>看守情况:</p><span>${partJunction.guardStatusName}</span></li>
	</ul>
</div>
<div class="imgAll">
       <div class="banner">
