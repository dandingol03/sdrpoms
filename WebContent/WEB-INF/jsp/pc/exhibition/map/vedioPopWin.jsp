...
<html>
  <head>
	...
	<style>
	button{width:200px;height:40px;background:red;}
	.ex_main_rightheader_pw4_header{
 	...
 	.ex_main_rightheader_pw4_content{
 		background: url(/sdrpoms/resources/image/exhibition/mappw/ex_map_pw_content.png) no-repeat;
    	background-size: 100% 100%;
 	}
 	.startClick{
	    width: 120px;
 	...
 	.video_bg{
	    background: url(/sdrpoms/resources/image/img_monitor.png) no-repeat;
	    background-size: 100% 100%;
	    width: 375px;
	    height: 190px;
	    position: absolute;
...
<body  onload="init()">
	<div class="ex_main_rightheader_pw4_header">
		<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_monitor.png" alt="" />
	...
	<div class="ex_main_rightheader_pw4_content">
		...
		<div style="width:375px;height:190px">
		...
		<div class="startClick" id="start">开始播放</div>
		<div class="startClick" id="end">停止播放</div>
		<div class="ex_main_rightheader_pw4_fristul">																								
			<ul>					
				<li><p>位置:</p><span>${video.descOrgName }${video.orgName }</span></li>
				<li><p>临近路线:</p>
					<span>
				...
				<li><p>权属单位:</p><span>${video.administration }</span></li>
				<li><p>功能描述:</p><span>${video.remark }</span></li>
			</ul>
		</div>
	</div>
	<!-- <button onclick="ButtonPtzDirection_onclick(1)">上</button>
	<button onclick="ButtonPtzDirection_onclick(2)">下</button>
	<button onclick="ButtonPtzDirection_onclick(3)">左</button>
	<button onclick="ButtonPtzDirection_onclick(4)">右</button> -->
    <script>     
		...
		$('#end').click(function(){
			...
			$('.video_bg').show();
			ButtonStopRealplayByHWND_onclick()
		});
		...
		function historyRealplay(cameraId) {
			var obj = parent.document.getElementById("DPSDK_OCX");
		...
		function insertRealplayHistory(cameraId) 
		{
			...
			//parent.document.getElementById("historyContent").appendChild(eleTr);
		}
		...
		function ButtonLogin_onclick() 
		{
			...
			console.log(gWndId);
			console.log("句柄"+gWndId);
			console.log("登录结果"+nRet);
			if(nRet == 0)
			{
//				var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId,  0, '1000003$1$0$0', 1, 1, 1);
//				var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId,  1, '1000003$1$0$0', 1, 1, 1);
//				var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId,  2, '1000003$1$0$0', 1, 1, 1);
//				var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId,  3, '1000003$1$0$0', 1, 1, 1);
				//var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId,  0, '1000003$1$0$0', 1, 1, 1);
				console.log("播放结果"+nRet);
			    bLogin = 1;
			}
...
//		    var nTransType = document.getElementById("selectTransType").value;
		...
		function ButtonStopRealplayByWndNo_onclick() 
		{
			...
			var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
		    var nRet=obj.DPSDK_StopRealplayByWndNo(gWndId, nWndNo);
		  
		    ShowCallRetInfo(nRet, "关闭视频");
		}
		...
		function ButtonPtzDirection_onclick(nDirects)
		{
			...
			//加载组织机构
			var obj = parent.document.getElementById("DPSDK_OCX");
			var ret=obj.DPSDK_LoadDGroupInfo()
			obj.DPSDK_GetDGroupStr()
		    var szCameraId = "${video.channel}";
		    var nDirect = nDirects;
		    var nStep = "1";
			setTimeout(function(){
				 ret=obj.DPSDK_PtzDirection(szCameraId,nDirects , nStep, 0)
			},1000)
