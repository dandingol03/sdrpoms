...
<body>
    <script type="text/javascript">
		...
		//添加隧道信息
		function addBaseInfoPartTunnel(){
			//验证字段
			var r = $('#baseInfoPartTunnelAddForm').form('validate');
			if(!r) {
				return false;
			}
			...
			var tempOutCome="";
			var tempFile="";
			$('#baseInfoPartTunnelAddForm').form('submit',{
				url : 'file/fileUpLoad',
					...
					if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
							...
							$('#baseInfoPartTunnelAddForm').form('submit',{
								...
								success : function(data) {
									if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
										tempFile=data;
										$.post("baseInfo/addBaseInfoPartTunnel?fileId="+tempOutCome+"&photos="+tempFile,
												$("#baseInfoPartTunnelAddForm").serializeArray(),function(data){
											closeWaitingWindow();
											$.messager.alert('提示',data.mes,'info');
										...
										$.post("baseInfo/addBaseInfoPartTunnel?fileId="+tempOutCome,
												$("#baseInfoPartTunnelAddForm").serializeArray(),function(data){
											closeWaitingWindow();
											$.messager.alert('提示',data.mes,'info');
											$('#baseInfoPartTunnelTable').datagrid('reload');	
						...
						$('#baseInfoPartTunnelAddForm').form('submit',{
							...
							success : function(data) {
								if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
									tempFile=data;	
									$.post("baseInfo/addBaseInfoPartTunnel?photos="+tempFile,
											$("#baseInfoPartTunnelAddForm").serializeArray(),function(data){
										closeWaitingWindow();
										$.messager.alert('提示',data.mes,'info');
										$('#baseInfoPartTunnelTable').datagrid('reload');	
									...
									$.post("baseInfo/addBaseInfoPartTunnel",
											$("#baseInfoPartTunnelAddForm").serializeArray(),function(data){
										closeWaitingWindow();
										$.messager.alert('提示',data.mes,'info');
										$('#baseInfoPartTunnelTable').datagrid('reload');	
	...
	<form id="baseInfoPartTunnelAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
		<tr>
					<tr>
			...
			<tr>
				...
				<td width="35%" align="left">
					<input class="easyui-filebox" name="uploadFileTwo" data-options="accept:'image/jpeg,image/gif,image/jpg,image/png',prompt:'请选择记录文件...',buttonText:'请选择文件',multiple:true" style='width:90%'/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addBaseInfoPartTunnel();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
