...
<body>
    <script type="text/javascript">
		...
		// 添加涵洞信息
		function addBaseInfoPartCulvert(){
			// 验证字段
			var r = $('#baseInfoPartCulvertAddForm').form('validate');
			if(!r) {
				return false;
			}
			...
			showWaitingWindow();
    		$('#baseInfoPartCulvertAddForm').form('submit',{
    			url : 'file/fileUpLoad',
    			...
    			success : function(data) {
    				if (typeof (data) != 'undefined') {
    					...
    					if (data != "-1" && data != "") {
    						var ps = "?fileId="+data;
    						console.log('===data==>', ps);
    						$.post("baseInfo/addPartCulvert"+ps,
    								$("#baseInfoPartCulvertAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartCulvertTable').datagrid('reload');
    						}); 
    					} else {
    						$.post("baseInfo/addPartCulvert",
    								$("#baseInfoPartCulvertAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartCulvertTable').datagrid('reload');
	...
	<form id="baseInfoPartCulvertAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addBaseInfoPartCulvert();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
