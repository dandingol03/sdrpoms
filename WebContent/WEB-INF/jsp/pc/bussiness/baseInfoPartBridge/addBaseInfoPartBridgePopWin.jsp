...
<body>
    <script type="text/javascript">
		...
		//添加车站信息
		function addBaseInfoPartBridge(){
			//验证字段
 			var r = $('#baseInfoPartBridgeAddForm').form('validate');
			if(!r) {
				return false;
			}
			...
			showWaitingWindow();
    		$('#baseInfoPartBridgeAddForm').form('submit',{
    			url : 'file/fileUpLoad',
    			...
    			success : function(data) {
    				if (typeof (data) != 'undefined') {
    					...
    					if (data != "-1" && data != "") {
    						var ps = "?fileId="+data;
    						console.log('===data==>', ps);
    						$.post("baseInfo/addBaseInfoPartBridge"+ps,
    								$("#baseInfoPartBridgeAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartBridgeTable').datagrid('reload');
    						}); 
    					} else {
    						$.post("baseInfo/addBaseInfoPartBridge",
    								$("#baseInfoPartBridgeAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartBridgeTable').datagrid('reload');
  	...
  	<!-- 弹出添加框 -->
	<form id="baseInfoPartBridgeAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addBaseInfoPartBridge();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
