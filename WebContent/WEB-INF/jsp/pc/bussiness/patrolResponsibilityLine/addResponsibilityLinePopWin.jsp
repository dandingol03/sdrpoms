...
<body>
    ...
    var endM =$('#endM')
		//添加
		function addResponsibilityLine(){
			//验证字段
			var r = $('#responsibilityLineAddForm').form('validate');
			if(!r) {
				return false;
			}
		...
		/* 	$.post("baseInfo/addResponsibilityLine",$("#responsibilityLineAddForm").serializeArray(),function(data){
				closeThirdWindow();
			...
			//
			showWaitingWindow();
    		$('#responsibilityLineAddForm').form('submit',{
    			url : 'file/fileUpLoad',
    			...
    			success : function(data) {
    				if (typeof (data) != 'undefined') {
    					if (data != "-1" && data != "") {
    						var ps = "?fileId="+data;
    						$.post("baseInfo/addResponsibilityLine"+ps,
    								$("#responsibilityLineAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeThirdWindow();
    							$('#responsibilityLineTable').datagrid('reload');
    						}); 
    					} else {
    						$.post("baseInfo/addResponsibilityLine",
    								$("#responsibilityLineAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeThirdWindow();
    							$('#responsibilityLineTable').datagrid('reload');
			...
			onChange: function () {
				...
				$.post("baseInfo/findRail",{railId:railId},function(data){
					...
					if(data.startStr!=undefined && data.endStr!=undefined){
						$("#textAdd").html("范围："+data.startStr+" 至   "+data.endStr);
					}
	...
	<form id="responsibilityLineAddForm" method="post" enctype="multipart/form-data"  class="operationPage">
		 ...
		 <table>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addResponsibilityLine();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
