...
<body>
    ...
    jQuery(function($){
		...
		function addDangerInfoTask(){
			
			var r = $('#dangerInfoTaskAddForm').form('validate');
			if(!r) {
				return false;
			}
			if(null==$("#dangerType").combobox("getValues")||$("#dangerType").combobox("getValues")==""){
				$.messager.alert('提示',"请选择隐患分类",'info');
				return;
			}
			...
			showWaitingWindow();
			$('#dangerInfoTaskAddForm').form('submit',{
				url : 'file/fileUpLoad',
				...
				success : function(data) {
					if (typeof (data) != 'undefined') {
						...
						if (data != "-1" && data != "") {
							var ps = "?photos="+data;
							console.log('===data==>', ps);
							$.post("patrol/addDangerInfo"+ps,
									$("#dangerInfoTaskAddForm").serializeArray(),function(data){
								$.messager.alert('提示',data.mes,'info');
								closeWindow();
								$('#dangerInfoTaskTable').datagrid('reload');
							}); 
						} else {
							$.post("patrol/addDangerInfo",
									$("#dangerInfoTaskAddForm").serializeArray(),function(data){
								$.messager.alert('提示',data.mes,'info');
								closeWindow();
								$('#dangerInfoTaskTable').datagrid('reload');
	...
	<form id="dangerInfoTaskAddForm" enctype="multipart/form-data"method="post" class="operationPage">
		...
		<table>
			...
			<tr>
				...
				<td width="35%" align="left">
					...
					style="height:20px;font-size:12px;width:39%;" > &nbspK&nbsp
					<input name="middleM" class="easyui-validatebox textbox" validType="number" 
					style="height:20px;font-size:12px;width:39%;" >&nbspM
				</td>
			</tr>
			<tr>
			...
			<tr> 
				...
				<td width="35%" align="left">
					<input name="uploadFile" class="easyui-filebox" data-options="accept:'image/jpeg,image/gif,image/jpg,image/png',prompt:'请选择记录文件...',buttonText:'请选择文件',multiple:true"  style="height:20px;font-size:12px;width:90%;">
				</td>
			</tr>
			<tr>
