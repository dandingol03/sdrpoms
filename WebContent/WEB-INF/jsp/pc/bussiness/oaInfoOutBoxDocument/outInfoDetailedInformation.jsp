...
<script type="text/javascript">
	...
	if(rowList[0].title){
		$('#title').append('<p style="width: 100%;line-height: 15px;font-size: 16px;color: black;text-align: center;">'+rowList[0].title+'</p>'); 
	}
	if(rowList[0].hairUnit!=undefined){
		$('#hairUnit').append('<p style="line-height: 7px;text-align: center;">发件单位：'+rowList[0].hairUnit+'</p>'); 
	}
	if(rowList[0].sendTime!=undefined){
		$('#sendTime').append('<p style="line-height: 7px;text-align: center;">发件日期：'+rowList[0].sendTime+'</p>'); 
	}
	if(rowList[0].receiveUserIdName!=undefined){
		$('#receiveUserIdName').append('<p style="line-height: 7px;text-align: center;">发件人：'+rowList[0].receiveUserIdName+'</p>'); 
	}
	if(rowList[0].editor!=undefined){
		$('#editor').append(rowList[0].editor); 
	}
	if(rowList[0].fileDownload!=undefined){
		$('#file').append('<p style="padding: 10px 10px;">'+rowList[0].fileDownload+','+'</p>'); 
	}}
	/* $('#fileName').append(rowList[0].fileName);   */
</script>
