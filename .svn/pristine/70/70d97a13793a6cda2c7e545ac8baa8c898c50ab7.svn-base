...
<body>
    <script type="text/javascript">
		...
		//添加护路宣传点位信息
		function addBaseInfoDefencePropaganda(){
			//验证字段
			...
			var r = $('#baseInfoDefencePropagandaAddForm').form('validate');
    		if(!r) {
    			return false;
    		}
    		$.post("baseInfo/checkProOrgId", $("#baseInfoDefencePropagandaAddForm").serializeArray(),function(data){
    			if(data.mes!=""){
    				$.messager.alert('提示',data.mes,'info');
    			}else{
    				showWaitingWindow();
    	    		$('#baseInfoDefencePropagandaAddForm').form('submit',{
    	    			url : 'file/fileUpLoad',
    	    			...
    	    			success : function(data) {
    	    				if (typeof (data) != 'undefined') {
    	    					...
    	    					if (data != "-1" && data != "") {
    	    						var ps = "?photos="+data;
    	    						console.log('===data==>', ps);
    	    						$.post("baseInfo/addBaseInfoDefencePropaganda"+ps,
    	    								$("#baseInfoDefencePropagandaAddForm").serializeArray(),function(data){
    	    							$.messager.alert('提示',data.mes,'info');
    	    							closeWindow();
    	    							$('#baseInfoDefencePropagandaTable').datagrid('reload');
    	    						}); 
    	    					} else {
    	    						$.post("baseInfo/addBaseInfoDefencePropaganda",
    	    								$("#baseInfoDefencePropagandaAddForm").serializeArray(),function(data){
    	    							$.messager.alert('提示',data.mes,'info');
    	    							closeWindow();
    	    							$('#baseInfoDefencePropagandaTable').datagrid('reload');
	...
	<form id="baseInfoDefencePropagandaAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
			<tr>
			...
			<tr>
				...
				<td width="35%" align="left">
					<input  name="uploadFile" class="easyui-filebox" data-options="accept:'image/jpeg,image/gif,image/jpg,image/png',prompt:'请选择记录文件...',buttonText:'请选择文件',multiple:true" style="height:23px;font-size:12px;width:91%;">
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addBaseInfoDefencePropaganda();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
