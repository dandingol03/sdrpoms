...
<body>
    <script type="text/javascript">
		...
		//添加信息
		function addOaInfoAnnouncement(){
			//验证字段
			var r = $('#oaAnnouncementAddForm').form('validate');
			if(!r) {
				return false;
			}
			showWaitingWindow();
			...
			var tempFile="";
			var tempThree="";
			$('#oaAnnouncementAddForm').form('submit',{
				url : 'file/fileUpLoad',
					...
					if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
									...
									if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
										...
										$('#oaAnnouncementAddForm').form('submit',{
											...
											success : function(data) {
												if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
													tempThree=data;
													$.post("oaInfo/addOaInfoAnnouncement?photos="+tempFile+"&bcPhotos="+tempOutCome+"&attachment="+tempThree,
															$("#oaAnnouncementAddForm").serializeArray(),function(data){
														closeWaitingWindow();
													...
													$.post("oaInfo/addOaInfoAnnouncement?photos="+tempOutCome+"&bcPhotos="+tempFile,
															$("#oaAnnouncementAddForm").serializeArray(),function(data){
														closeWaitingWindow();
														$.messager.alert('提示',data.mes,'info');
														$('#oaAnnouncementTable').datagrid('reload');	
										...
										$.post("oaInfo/addOaInfoAnnouncement?photos="+tempOutCome,
												$("#oaAnnouncementAddForm").serializeArray(),function(data){
											closeWaitingWindow();
											$.messager.alert('提示',data.mes,'info');
											$('#oaAnnouncementTable').datagrid('reload');	
						...
						$('#oaAnnouncementAddForm').form('submit',{
							...
							success : function(data) {
								if (typeof (data) != 'undefined'&&data != "-1" && data != "") {
									tempFile=data;	
									$.post("oaInfo/addOaInfoAnnouncement?bcPhotos="+tempFile,
											$("#oaAnnouncementAddForm").serializeArray(),function(data){
										closeWaitingWindow();
										$.messager.alert('提示',data.mes,'info');
										$('#oaAnnouncementTable').datagrid('reload');	
									...
									$.post("oaInfo/addOaInfoAnnouncement",
											$("#oaAnnouncementAddForm").serializeArray(),function(data){
										closeWaitingWindow();
										$.messager.alert('提示',data.mes,'info');
										$('#oaAnnouncementTable').datagrid('reload');	
	...
	<form id="oaAnnouncementAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
			<tr>
			...
			<tr>
				...
				<td width="35%" align="left">
					<input class="easyui-filebox" name="uploadFileTwo" data-options="accept:'image/jpeg,image/gif,image/jpg,image/png',prompt:'请选择记录文件...',buttonText:'请选择文件',multiple:true,required:true" style='width:90%'/>
					<a style="color:red">*</a>
				</td>
			</tr>
			<tr>
				...
				<td width="35%" align="left">
					<input class="easyui-filebox"  name="uploadFileThree" data-options="prompt:'请选择记录文件...',buttonText:'请选择文件',multiple:false" style='width:90%;'/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addOaInfoAnnouncement();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
