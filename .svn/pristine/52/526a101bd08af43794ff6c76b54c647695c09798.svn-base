...
<style>
 	/* .ex_main_rightheader_pw4_header{
	 	background:url(${ctx}/resources/image/exhibition/map/legend/ex_map_policeStation.png) left 10px top 29px #00b9f6 no-repeat;
 	} */
 	.ex_main_rightheader_pw4_header{
...
<div class="ex_main_rightheader_pw4_header">
	<img src="${ctx}/resources/image/exhibition/map/legend/ex_map_policeStation.png">
	<span>${defencePoliceHouse.name}</span>
	<div class="ex_main_rightheader_closepw4" onclick="closePw()"></div>
</div>
<div class="ex_main_rightheader_pw4_content">
	<div id="igs">
		<c:choose>
		   <c:when  test="${empty defencePoliceHouse.designProfiles}"> 
			   	<img src="${ctx}/resources/image/opaction.png"style="width: 100%;height: 100%;background: red;">
		   </c:when>
		   <c:otherwise>
		   		<c:forEach items="${defencePoliceHouse.designProfiles}"  var="designProfiles">
		   			<a class="ig" href="#">
						<img src="${ctx}/file/showPicFile?fileId=${designProfiles}"/>
						<div class="imgNumber">${defencePoliceHouse.photosNum}</div>
					</a>
		   		</c:forEach>
   	...
   	<ul class="ex_main_rightheader_pw4_fristul">
		<li><p>地址:</p><span>${defencePoliceHouse.adress}</span></li>
		<li><p>管辖范围:</p><span>${defencePoliceHouse.defenceBroadcast}</span></li>	
		<li><p>联系电话:</p><span>${defencePoliceHouse.telephone}</span></li> 
	</ul>
</div>
<div class="imgAll">
       <div class="banner">
