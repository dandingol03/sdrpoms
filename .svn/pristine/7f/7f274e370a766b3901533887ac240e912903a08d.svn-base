...
<body>
    <script type="text/javascript">
		function updateBaseInfoDefencePoliceHouse(){
			var r = $('#baseInfoDefencePoliceHouseUpdateForm').form('validate');
			if(!r) {
				return false;
			}
			...
			showWaitingWindow();
    		$('#baseInfoDefencePoliceHouseUpdateForm').form('submit',{
    			url : 'file/fileUpLoad',
    			...
    			success : function(data) {
    				if (typeof (data) != 'undefined') {
    					...
    					if (data != "-1" && data != "") {
    						var ps = "?fileId="+data;
    						console.log('===data==>', ps);
    						var addNumber=$("#baseInfoDefencePoliceHouseUpdateForm").serializeArray();
    						addNumber.splice(1,1);
    						$.post("baseInfo/updateBaseInfoDefencePoliceHouse"+ps,addNumber,function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    						...
    						var addNumber=$("#baseInfoDefencePoliceHouseUpdateForm").serializeArray();
    						$.post("baseInfo/updateBaseInfoDefencePoliceHouse",addNumber,function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoDefencePoliceHouseTable').datagrid('reload');
    						}); 
	...
	<form id="baseInfoDefencePoliceHouseUpdateForm" enctype="multipart/form-data" method="post" class="operationPage">
		...
		<table>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="updateBaseInfoDefencePoliceHouse();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
