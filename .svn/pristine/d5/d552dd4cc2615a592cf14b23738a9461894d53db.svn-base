...
<body>
    <script type="text/javascript">
		...
		//添加道口信息
		function addBaseInfoPartJunction(){
			//验证字段
			var r = $('#baseInfoPartJunctionAddForm').form('validate');
			if(!r) {
				return false;
			}
			...
			showWaitingWindow();
    		$('#baseInfoPartJunctionAddForm').form('submit',{
    			url : 'file/fileUpLoad',
    			...
    			success : function(data) {
    				if (typeof (data) != 'undefined') {
    					...
    					if (data != "-1" && data != "") {
    						var ps = "?fileId="+data;
    						console.log('===data==>', ps);
    						$.post("baseInfo/addBaseInfoPartJunction"+ps,
    								$("#baseInfoPartJunctionAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartJunctionTable').datagrid('reload');
    						}); 
    					} else {
    						$.post("baseInfo/addBaseInfoPartJunction",
    								$("#baseInfoPartJunctionAddForm").serializeArray(),function(data){
    							$.messager.alert('提示',data.mes,'info');
    							closeWindow();
    							$('#baseInfoPartJunctionTable').datagrid('reload');
	...
	<form id="baseInfoPartJunctionAddForm" enctype="multipart/form-data" method="post" class="operationPage">
		<table>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a onclick="addBaseInfoPartJunction();" class="easyui-linkbutton" iconCls="icon-save">保存</a>
				</td>
			</tr>
		</table>
	</form>
