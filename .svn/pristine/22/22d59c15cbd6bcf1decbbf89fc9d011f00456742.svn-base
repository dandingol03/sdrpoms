        ...
        //服务器定时发回在线的成员列表信息
            connection.socket.on('return-userlist', function (responseCode, data) {
            	...
            	for(var i=0;i<data.length;i++){
            		if(data[i].type=="mobile" && data[i].isTeamMember=="1"){
            			mobile++;
            		}
            	}
            	$('#teamNumber').html(mobile);
            	//干部
            	mobileD=0;
              	for(var i=0;i<data.length;i++){
              		if(data[i].type=="mobile" && data[i].isTeamMember=="0"){
              			mobileD++;
              		}
              	}
              	$('#teamCadreNumber').html(mobileD);
            	if(teamMember){ 
	            	...
	            	for(var i=0;i<data.length;i++){
	            		...
	            		if(data[i].type=="mobile" && data[i].isTeamMember=="1" ){
	            			var FullTimeMemberFeature = new ol.Feature({
		    					geometry: new ol.geom.Point([data[i].lng,data[i].lat]),
		    					id:data[i].userId,
		    				});
		    				FullTimeMemberFeature.setStyle(keyPersonStyle);
            	...
            	if(teamCadre){ 
	            	...
	            	for(var i=0;i<data.length;i++){
	        			...
	        			continue;
	            		if(data[i].type=="mobile" && data[i].isTeamMember=="0" ){
	            			var FullTimeAFeature = new ol.Feature({
		    					geometry: new ol.geom.Point([data[i].lng,data[i].lat]),
		    					id:data[i].userId,
		    				});
	            			FullTimeAFeature.setStyle(keyPersonStyle);
		    				BJAppLayer.getSource().addFeature(FullTimeAFeature);
	            		}
	            	}
...
//                connection.filesContainer.insertBefore(div, connection.filesContainer.firstChild);
               connection.filesContainer. append(div); 
                if (!file.remoteUserId) {
    ...
    (function (connection) {
        ...
        connection.notify = function (remoteUserId, localUserid) {
            if (connection.socket) {
            	console.log("notify++++++++++++++++++"+remoteUserId,localUserid)
                setTimeout(function () {
